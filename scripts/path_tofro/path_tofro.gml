...
var _target_x = path_get_point_x(_path, path_position);
var _target_y = path_get_point_y(_path, path_position);
...
if(_distance_to_point < _max_distance) {
    path_position += sign(_dir);
    if(path_position > path_get_number(_path)-1) { path_position -= 2; my_path_dir *= -1;}
    if(path_position < 0) { path_position += 2; my_path_dir *= -1;}
} 
